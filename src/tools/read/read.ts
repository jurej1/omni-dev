...
export const readTool = tool({
  ...
  execute: async (params) => {
    ...
    const stat = fs.statSync(filepath) ?? undefined;

    if (!stat) {
      const dir = path.dirname(filepath);
      const base = path.basename(filepath);

      let filesSug = fs.readdirSync(dir);

      const suggestions = filesSug
        .filter(
          (entry) =>
      ...
      const entries = await Promise.all(
        dirents.map(async (dirent) => {
          ...
          if (dirent.isSymbolicLink()) {
            const target = fs.statSync(path.join(filepath, dirent.name));
    ...
    const mime = Bun.file(filepath).type;
    const isImage =
        ...
        attachments: [
          {
            type: "file",
            mime,
            url: `data:${mime};base64,${Buffer.from(fs.readFileSync(filepath)).toString("base64")}`,
          },
        ],
      };
    }
...
async function isBinaryFile(
  ...
  try {
    ...
    const bytesRead = fs.readSync(fd, bytes, 0, sampleSize, 0);
